...
def module_command(module: str, *args: str) -> list[str]:
    """Build a command that works both from source and from launcher.exe."""
...
def fast_scraper_command(headless: bool) -> list[str]:
    command = module_command("pipeline.fast_runner")
    command.append("--headless" if headless else "--headed")
    return command


def scraper_profile_command() -> list[str]:
    """Launch the persistent Chrome profile used by the fast scrapers."""
    return module_command("New_Scrapers.profile_login")
