...
from processing.platform_paths import NORMALIZED_PRODUCTS, PROCESS_STATUS
...
tuples = read_json(NORMALIZED_PRODUCTS, {"summary": {}})

running = [site for site, record in status.items() if record.get("running")]
cols = st.columns(3)
cols[0].metric("Running jobs", len(running))
cols[1].metric("Tuple count", tuples.get("summary", {}).get("normalized_products", 0))
cols[2].metric("Master rows", tuples.get("summary", {}).get("master_rows", 0))
