...
def update_site_status(site: str, fields: dict[str, Any]) -> dict[str, Any]:
    with _LOCK:
        ...
        if current > 0 and total > current and record.get("started_at"):
            try:
                elapsed = max(0.0, (datetime.now() - datetime.fromisoformat(record["started_at"])).total_seconds())
                record["elapsed_seconds"] = round(elapsed)
                record["eta_seconds"] = round((elapsed / current) * (total - current))
            except (TypeError, ValueError, ZeroDivisionError):
                pass
        record["percentage"] = round((current / total) * 100) if total else 0
